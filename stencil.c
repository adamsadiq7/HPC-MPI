...
int main(int argc, char *argv[]) {
  ...
  // creating buffers to hold segment images
  int segmentSize = nx*ny/16;
  float *buffer = malloc(sizeof(float)* segmentSize);
  float *bufferTemp = malloc(sizeof(float)* segmentSize);

  //scatter image into 16 processors
  ...
  if (rank == MASTER){
    result = malloc(sizeof(float)*nx*ny);
  }
...
float *getRow(float* inputArray,float * segment, int start,int end){
  int j = 0;
  for( int i =start ; i<= end; i++){
    segment[j] = inputArray[i];
    j++
  }
...
void stencil(const int nx, const int ny, float *restrict image, float *  restrict tmp_image, int rank) {
    ...
    float *topRowRecv = malloc(sizeof(float)*nx);
    float *bottomRowRecv = malloc(sizeof(float)*nx);
    ...
    topRowStart = 0;
    topRowEnd = nx;
    bottomRowStart = nx*ny - nx;
    bottomRowEnd = nx*ny -1;

    MPI_Status *status;

    if (rank == MASTER){
      bottomRowSend = getRow(image, bottomRowSend, bottomRowStart, bottomRowEnd);

      MPI_Send(bottomRowSend, nx, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD);
      MPI_Recv(bottomRowRecv, nx, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD, status);
    ...
    {
      topRowSend = getRow(image, topRowSend, topRowStart, topRowEnd);

      MPI_Send(topRowSend, nx, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD);
      MPI_Recv(topRowRecv, nx, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD, status);
    }
